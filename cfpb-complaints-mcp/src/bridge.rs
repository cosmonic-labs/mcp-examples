...
pub mod outbound {
    ...
    /// with `MCP_OUTBOUND_MAX_BYTES`. A wide aggregation or a large `size=`
    /// page from the CFPB API can exceed the 4 MiB request-side default, so
    /// tools that expect large payloads can raise it.
